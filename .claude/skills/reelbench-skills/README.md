...
| `rapid-transformation-intro.md` | A rapid multi-look/multi-world scene-introduction pattern (imported, not project-derived): simultaneous outfit+location+composition change per beat, a never-repeat-typography-style rule, dual-script per-beat titling, and a finale collage that visually recalls every prior beat |
| `combat-environment-reactivity.md` | How a fight scene's own VFX (glow, impacts, explosions) should visibly light its environment — the effect as motivated/practical light, smoke/dust as the volumetric medium its light travels through, debris as discrete particles, flickering shadows, a Light Flash at peak impact, and bounce light onto nearby surfaces — named techniques from `melies-cinematic-library`, distinct from damage persistence (`action-sequence-craft.md`) and a single effect's own internal structure (`minimax-h3-prompt-writing`) |
