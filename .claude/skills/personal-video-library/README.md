...
0. **Settle the story first** (if there's character/story stakes) — hand off to `story-wizard`
   for its Socratic elicitation (action/presentation/arc/hook/ending) before anything technical
   starts. Skip only for a pure technique/style request with no narrative content.
...
3. **Force the creative addition** — pull 2-3 named techniques from `melies-cinematic-library`
   ...
   user explicitly, then route the actual prompt-writing with the measured brief, the chosen
   additions, and the Story Brief from step 0 as explicit instructions.
...
| Skill | Answers |
|---|---|
| `story-wizard` | "What is the new piece actually about — action, hook, arc, ending?" (runs first, if there's story stakes) |
